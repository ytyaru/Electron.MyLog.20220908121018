class MyLogDb {
    ...
    async insert(content, address=null) {
        if (!content) { alert('つぶやく内容をテキストエリアに入力してください。'); return null; }
        if (this.LENGTH < content.length) { alert(`つぶやく内容は${this.LENGTH}字以内にしてください。`); return null; }
        const match = content.match(/\r\n|\n/g)
        if (match && this.LINE < match.length) { alert(`つぶやく内容は${this.LINE}行以内にしてください。`); return null; }
